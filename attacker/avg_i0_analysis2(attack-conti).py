...
# 讀取日誌資料
with open('/Users/yq/Documents/analysis/avg-i0_attack-conti.py', 'r', encoding='utf-8') as file: #Users/yq/Documents/analysis/avg-i0_normal.py'
    log_data = file.read()
...
fig.suptitle('NR gNB I0 測量分析 - 攻擊場景下的 avg_I0 變化趨勢', fontsize=16, fontweight='bold')
...
# 設定合理的Y軸範圍，聚焦主要資料範圍
main_values = [v for v in avg_i0_values if v <= 70]  # 調整極值過濾閾值
if main_values:
    y_min = min(main_values) - 2
    y_max = max(main_values) + 3  
    ax1.set_ylim(y_min, y_max)
    
    # 動態調整參考線，基於實際資料分佈
    sorted_values = sorted(avg_i0_distribution.keys())
    # 選擇出現頻率最高的前3個值作為參考線
    top_values = sorted(avg_i0_distribution.keys(), key=lambda x: avg_i0_distribution[x], reverse=True)[:3]
    
    for value in top_values:
        if value <= y_max:  # 只顯示在可見範圍內的參考線
            percentage = (avg_i0_distribution[value] / len(avg_i0_values)) * 100
            ax1.axhline(y=value, color='gray', linestyle='--', alpha=0.5, linewidth=0.8)
            # 調整文字位置避免重疊
            y_pos = (value - y_min) / (y_max - y_min)
            ax1.text(0.02, y_pos, f'{value}dB ({percentage:.1f}%)', 
                    transform=ax1.transAxes, fontsize=9, color='gray',
                    bbox=dict(boxstyle="round,pad=0.2", facecolor='white', alpha=0.7))
else:
    # 如果沒有主要值，使用全範圍
    y_min = min(avg_i0_values) - 2
    y_max = max(avg_i0_values) + 3
    ax1.set_ylim(y_min, y_max)
...
# 動態設定Y軸範圍
prach_min = min(prach_i0_values)
prach_max = max(prach_i0_values)
prach_range = prach_max - prach_min
y_margin = max(2, prach_range * 0.05)  # 至少2dB margin，或5%的範圍
ax2.set_ylim(prach_min - y_margin, prach_max + y_margin)

# 添加平均值和中位數參考線
mean_prach = statistics.mean(prach_i0_values)
median_prach = statistics.median(prach_i0_values)
ax2.axhline(y=mean_prach, color='darkred', linestyle='--', alpha=0.7, linewidth=1, label='平均值')
ax2.axhline(y=median_prach, color='orange', linestyle=':', alpha=0.7, linewidth=1, label='中位數')
...
ax2.text(0.02, 0.85, f'中位數: {median_prach:.1f} dB', transform=ax2.transAxes, 
         bbox=dict(boxstyle="round,pad=0.3", facecolor='lightyellow', alpha=0.8), fontsize=9)
ax2.text(0.02, 0.75, f'範圍: {prach_range:.1f} dB', transform=ax2.transAxes, 
         bbox=dict(boxstyle="round,pad=0.3", facecolor='lightblue', alpha=0.8), fontsize=9)
...
# 4. avg_I0 極值事件分析 - 動態設定閾值
avg_range = max(avg_i0_values) - min(avg_i0_values)
extreme_threshold = min(avg_i0_values) + avg_range * 0.7  # 設定為範圍的70%作為極值閾值
extreme_threshold = int(extreme_threshold)  # 取整數
...
if extreme_values:
    ...
    # 設定Y軸範圍顯示完整的極值範圍
    if extreme_values:
        y_min_extreme = min(extreme_values) - 3
        y_max_extreme = max(extreme_values) + 3
        ax4.set_ylim(y_min_extreme, y_max_extreme)
    
    # 添加統計資訊
    extreme_percentage = (len(extreme_values) / len(avg_i0_values)) * 100
    ax4.text(0.02, 0.95, f'極值次數: {len(extreme_values)} ({extreme_percentage:.1f}%)', transform=ax4.transAxes,
    ...
    ax4.text(0.02, 0.85, f'極值範圍: {min(extreme_values)}-{max(extreme_values)} dB', 
             ...
             bbox=dict(boxstyle="round,pad=0.3", facecolor='yellow', alpha=0.7), fontsize=9)
    ax4.text(0.02, 0.75, f'閾值: ≥{extreme_threshold} dB', 
             transform=ax4.transAxes,
             bbox=dict(boxstyle="round,pad=0.3", facecolor='orange', alpha=0.7), fontsize=9)
    
    ax4.legend()
else:
    ax4.text(0.5, 0.5, f'無極值事件 (閾值: {extreme_threshold} dB)', transform=ax4.transAxes, 
             ha='center', va='center', fontsize=14)
    ax4.set_title(f'avg_I0 極值事件分析 (閾值: ≥{extreme_threshold} dB)')
    ax4.set_xlabel('Frame.Slot')
    ax4.set_ylabel('avg_I0 (dB)')
...
# 儲存圖表
plt.savefig('/Users/yq/Documents/analysis/attacker/avg_i0_analysis2(attack-conti).png', dpi=300, bbox_inches='tight')
print(f"\n圖表已儲存為: avg_i0_analysis2(attack-conti).png")

plt.show()

# 創建第二個圖表：詳細的avg_I0分析
fig2, ((ax5, ax6), (ax7, ax8)) = plt.subplots(2, 2, figsize=(15, 12))
fig2.suptitle('攻擊場景 - avg_I0 詳細分析', fontsize=16, fontweight='bold')
...
# 儲存第二個圖表
plt.savefig('/Users/yq/Documents/analysis/attacker/avg_i0_detailed_analysis2(attack-conti).png', dpi=300, bbox_inches='tight')
print(f"詳細分析圖表已儲存為: avg_i0_detailed_analysis2(attack-conti).png")
