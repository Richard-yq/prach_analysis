...
def main():
    ...
    for i in range(len(x)):
        ...
        if detected[i] != accepted[i]:
            ax.text(x[i], detected[i] + 5, f"{detected[i]}", color='#34495e', ha='center', va='bottom', fontsize=9, fontweight='bold')
        if dropped[i] > 0:
            ax.text(x[i], dropped[i] - 15, f"{dropped[i]}", color='#e74c3c', ha='center', va='top', fontsize=9, fontweight='bold')
    ...
    ax.yaxis.set_major_locator(FixedLocator([0, 2, 4, 10, 50, 100, 150]))
    ax.yaxis.set_major_formatter(ScalarFormatter())
    ax.set_ylim(-1, 200)

    ax.legend(loc='upper right', bbox_to_anchor=(1.0, 0.8), fontsize=11)
    
    plt.tight_layout()
    out = OUT_DIR / "resource_exhaustion_concept_fdm24_symlog.png"
